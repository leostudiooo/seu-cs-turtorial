/*
本来以为这个题挺简单的，然后用主程序本身作为输入，结果发现怎么 debug 都不对，在遇到字符串内包裹双斜杠这种情况的时候总是不能按照预期运行，直接从双斜杠就把那一整行都删掉了。然后又想到了行末注释，还有各种直接用字符串操作不好处理（但是编辑器基于状态机/正则匹配的语法高亮就能做）的 edge case，比如行内的块注释（见 ln 102）。光是今天这个作业我就新建了三个 branch，即使是最简单的不考虑括号包裹等各种问题的版本也会出问题。但是我认为这一版的设计思路是比较完善的。
然后让 GPT-4o 生成了一个版本 (attached: example_by_gpt.cpp)，那个能正常跑 (输出文件：output.cpp)。看了一下，是一个基于状态机的解法，但是状态机也太超纲了。
